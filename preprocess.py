...
class Preprocess(nn.Module):
    def __init__(self, device: str, opt: argparse.Namespace, hf_key: str = None):
        super().__init__()
...
def prep(opt):
    ...
    add_dict_to_yaml_file(
        os.path.join(opt.save_dir, "inversion_prompts.yaml"),
        Path(opt.data_path).stem,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="for non-square videos, we recommend using 672 x 384 or 384 x 672, aspect ratio 1.75",
    )
