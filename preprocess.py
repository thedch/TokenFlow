...
class Preprocess(nn.Module):
    def __init__(self, device, opt, hf_key=None):
        ...
        self.text_encoder = CLIPTextModel.from_pretrained(
            model_key, subfolder="text_encoder", revision="fp16", torch_dtype=torch.float16
        ).to(self.device)
...
def prep(opt):
    ...
    timesteps_to_save, num_inference_steps = get_timesteps(
        toy_scheduler, num_inference_steps=opt.save_steps, strength=1.0, device=device
    )

    seed_everything(1)

    save_path = os.path.join(
        opt.save_dir, f'sd_{opt.sd_version}', Path(opt.data_path).stem, f'steps_{opt.steps}', f'nframes_{opt.n_frames}'
    )
    os.makedirs(os.path.join(save_path, f'latents'), exist_ok=True)
    add_dict_to_yaml_file(
        os.path.join(opt.save_dir, 'inversion_prompts.yaml'), Path(opt.data_path).stem, opt.inversion_prompt
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='data/woman-running.mp4')
    parser.add_argument(
        '--H',
        type=int,
        default=512,
