...
class TokenFlow(nn.Module):
    def __init__(self, config):
        ...
        self.keyframes_path = [
            os.path.join(config["data_path"], "%05d.jpg" % idx) for idx in range(self.config["n_frames"])
        ]
        if not os.path.exists(self.keyframes_path[0]):
            self.keyframes_path = [
                os.path.join(config["data_path"], "%05d.png" % idx) for idx in range(self.config["n_frames"])
            ]
        # load frames
