...
from tokenflow_utils import (load_source_latents_t, register_batch_idx,
                             register_extended_attention, register_pivotal,
                             register_time, set_tokenflow)
...
class TokenFlow(nn.Module):
    ...
    def batched_denoise_step(self, x, t, indices):
        ...
        denoised_latents = []
        pivotal_idx = (
            torch.randint(batch_size, (len(x) // batch_size,))
            + torch.arange(0, len(x), batch_size)
